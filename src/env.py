...
def get_minimum():
    cal = np.zeros((1, 2))  # Initialize the array to hold the data

    for data_num in range(TXT_NUM):  # Loop through the data files
        data_name = str("%03d" % (data_num + 1))  # Format the data number
        file_name = "KAIST" + "_30sec_" + data_name + ".txt"  # Construct file name
        file_path = "C:/Users/harih/Downloads/Resources-Allocation-in-The-Edge-Computing-Environment-Using-Reinforcement-Learning-master/Resources-Allocation-in-The-Edge-Computing-Environment-Using-Reinforcement-Learning-master/data/KAIST/" + file_name  # Construct full path
        ...
        try:
            ...
            line_num = len(f1)  # Get the number of lines
            data = np.zeros((line_num, 2))  # Initialize a numpy array to hold the data

            for index, line in enumerate(f1):
                # Make sure the line has enough elements
                split_line = line.split()
                if len(split_line) >= 3:  # Ensure there are at least 3 elements to avoid index error
                    data[index][0] = float(split_line[1])  # x
                    data[index][1] = float(split_line[2])  # y

            # Append the data to the cal array
            cal = np.vstack((cal, data))

        except Exception as e:
            print(f"Error reading file {file_path}: {e}")

    # Return the minimum values from each column
...
def proper_edge_loc(edge_num, TXT_NUM):  # Assuming TXT_NUM is passed as an argument
    # Initialize the e_l (edge locations)
    ...
    for base in range(0, group_num * edge_num, group_num):
        cal = np.zeros((0, 2))  # Initialize 'cal' for stacking data

        for data_num in range(base, base + group_num):
            data_name = str("%03d" % (data_num + 1))  # data_num should be dynamic
            ...
            try:
                ...
                # Get line_num and initialize data array
                line_num = len(f1)
                ...
                for index, line in enumerate(f1):
                    split_line = line.split()
                    if len(split_line) >= 3:  # Ensure there are enough elements in the line
                        data[index][0] = float(split_line[1])  # x
                        data[index][1] = float(split_line[2])  # y

                # Stack the collected data
                if data_num == base:  # Initialize cal with the first dataset
                    cal = data
                else:
                    cal = np.vstack((cal, data))
...
class UE():
    def __init__(self, user_id, data_num):
        ...
        self.num_step = 0  # the number of steps

        # Construct file path
        data_name = str("%03d" % (data_num + 1))  # Assuming data_num is defined
        file_name = "KAIST" + "_30sec_" + data_name + ".txt"  # Assuming LOCATION is defined
        file_path = "C:/Users/harih/Downloads/Resources-Allocation-in-The-Edge-Computing-Environment-Using-Reinforcement-Learning-master/Resources-Allocation-in-The-Edge-Computing-Environment-Using-Reinforcement-Learning-master/data/KAIST/" + file_name
        ...
        try:
            ...
            # Calculate num_step
            self.num_step = len(f1) * 30  # Each line corresponds to 30 seconds
            self.mob = np.zeros((self.num_step, 2))  # Initialize mob array
            ...
            for line in f1:
                ...
                if len(split_line) >= 3:  # Ensure there are enough elements in the line
                    for sec in range(30):
                        self.mob[now_sec + sec][0] = float(split_line[1])  # x
                        self.mob[now_sec + sec][1] = float(split_line[2])  # y
                    now_sec += 30
            self.loc[0] = self.mob[0]  # Set initial location
...
class Env():
    ...
    def reset(self, TXT_NUM):
        ...
        # server
        self.E = []
        e_l = proper_edge_loc(self.edge_num, TXT_NUM)  # Pass TXT_NUM here
